...
class _AddProductState extends State<AddProduct> {
  ...
  Future<Null> processAddProduct() async {
    if (formKey.currentState!.validate()) {
       if (typeproduct == null) {
            print('Non Choose Type product');
            MyDialog().normalDialog(context, 'ยังไม่ได้เลือก ชนิดของสินค้า ',
                'กรุณา เลือก ที่ ชนิดของสินค้า ที่ ต้องการ ');
          }
      bool checkFile = true;
        ...
        for (var item in files) {
          ...
          await Dio().post(apiSaveProduct, data: data).then((value) async {
            ...
            if (loop >= files.length) {
              ...
              // print('### id = $IdProduct');
              // print(
              //     '### product = $NameProduct, price = $PriceProduct, detail = $DetailProduct');
              // print('### image ==> $Image');
  ...
  Widget buildProductPrice(BoxConstraints constraints) {
    return Container(
      ...
      child: TextFormField(
        ...
        validator: (value) {
          if (value!.isEmpty) {
            return 'กรุณากรอก Poduct Price ในช่องว่างด้วย';
          } else {
            return null;
          }
        },
  ...
  Widget buildProductDetail(BoxConstraints constraints) {
    return Container(
      ...
      child: TextFormField(
        ...
        validator: (value) {
          if (value!.isEmpty) {
            return 'กรุณากรอก Product Detail ในช่องว่างด้วย';
          } else {
            return null;
          }
        },
